...
/*   Updated: 2018/11/25 09:46:27 by jnoe             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**makelst(t_list **lst, int fd)
{
	...
	nlst = *lst;
	while (nlst->next != NULL && nlst->fd != fd)
		nlst = nlst->next;
	if (nlst->next == NULL && nlst->fd != fd)
	{
		nlst->next = malloc(sizeof(t_list));
		nlst = nlst->next;
		nlst->fd = fd;
		nlst->str = NULL;
		nlst->next = NULL;
...
int		get_next_line(const int fd, char **line)
{
	...
	*line = NULL;
	...
	if ((*line = (char *)malloc(1)) == NULL)
		return (-1);
	*line[0] = '\0';
	return (check_endline(fd, makelst(&lst, fd), line));
}
